...
struct Leaderboard {
    ...
    func printStatistics(puzzleIndex: Int?) {
        do {
            let leaderboardData = try Data(contentsOf: URL(string: "https://adventofcode.com/2022/leaderboard/private/view/\(ID).json")!)
            let leaderboardJSON = try (JSONSerialization.jsonObject(with: leaderboardData, options: []) as! [String : Any])
            guard let membersJSON = leaderboardJSON["members"] as? [String : Any] else { return }
            
            var users: [User] = []
            var puzzleResultsByIndex: [Int : PuzzleResult] = [:]
            
            for (userID, userJSON) in membersJSON {
                let userJSONDictionary = userJSON as! [String : Any]
                let user = User(ID: Int(userID)!, jsonDictionary: userJSONDictionary)
                users.append(user)
                
                let times = userJSONDictionary["completion_day_level"] as! [String : [String : [String : Int]]]
                for (indexString, dictionary) in times {
                    let index = Int(indexString)!
                    var result = puzzleResultsByIndex[index] ?? PuzzleResult(puzzleIndex: index)
